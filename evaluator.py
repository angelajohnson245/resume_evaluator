...
if not my_api_key:
    raise ValueError("API key is required. Please set the GROQ_API_KEY environment variable.")
...
def parse_job_description(job_description):
    ...
    system_prompt = f"""
    You are an expert HR assistant. Extract structured information from the job description.
    Return ONLY valid JSON matching this schema: {jobd_schema}
    Do not return the schema outline, only the populated fields. If data is missing, use null or empty lists.
    """
...
def parse_resume(resume_text):
    ...
    7. Do NOT return "properties", "title", or "type".
    8. Do NOT return a list.
    9. Always include all required fields.
    10. If a field has no data, return null or empty lists.
    """
    user_prompt = f"""
...
def final_score(job,resume):
    ...
    - Do NOT return "properties", "title", or "type".
    - Do NOT return a list.
    - Always include all required fields.
    - If a field has no data, return null or an empty list.

    Fill each field based on the comparison.
...
if __name__ == "__main__":
    ...
    for file_path in resume_folder.iterdir():
        ...
        try:
            ...
            time.sleep(2)  # Rate limiting safety gap
            
            evaluation = final_score(job, parsed_resume)
            time.sleep(2)
            ...
            print(f"Scored: {evaluation.score}%")
            
        except Exception as e:
            print(f"Error processing {file_path.name}: {e}")

    # Sorting and safely ranking pools
    ...
    if total_candidates == 0:
        ...
        print("\n=== LOWEST 2 CANDIDATES ===")
        # Avoid overlapping outputs if pool size is small
        worst_candidates = all_results[-2:]
